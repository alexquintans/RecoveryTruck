...
class PagBankAdapter(PaymentAdapter):
    """🏧 Adaptador para integração com PagBank com impressão completa"""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(f"🏧 PagBankAdapter initialized for merchant {self.merchant_id}")
    ...
    async def _fetch_transaction_details(self, transaction_id: str) -> Dict[str, Any]:
        """🔍 Busca detalhes completos da transação na API PagBank"""
        try:
            response = await self.client.get(f"/payments/{transaction_id}")
            response.raise_for_status()
            ...
            # Enriquece dados com informações específicas do PagBank
            transaction_data["merchant"] = {
                "name": self.config.get("merchant_name", "PagBank Merchant"),
                "cnpj": self.config.get("merchant_cnpj", "00000000000000"),
                "address": self.config.get("merchant_address")
            }
            
            logger.info(f"🔍 PagBank transaction details fetched: {transaction_id}")
            return transaction_data
            
        except Exception as e:
            logger.error(f"❌ Error fetching PagBank transaction {transaction_id}: {e}")
            raise
