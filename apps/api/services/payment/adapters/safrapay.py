...
class SafraPayAdapter(PaymentAdapter):
    """🏦 Adaptador para integração com SafraPay com impressão completa"""
    
    def __init__(self, config: Dict[str, Any]):
        """Inicializa o adaptador com as configurações do tenant."""
        super().__init__(config)  # Inicializa impressora
        
        self.api_url = config.get("api_url", "https://api.safrapay.com.br/v1")
        ...
        logger.info(f"🏦 SafraPayAdapter initialized for merchant {self.merchant_id}")
    ...
    async def _fetch_transaction_details(self, transaction_id: str) -> Dict[str, Any]:
        """🔍 Busca detalhes completos da transação na API SafraPay"""
        try:
            response = await self.client.get(f"/payments/{transaction_id}")
            response.raise_for_status()
            ...
            # Enriquece dados com informações específicas do SafraPay
            transaction_data["merchant"] = {
                "name": self.config.get("merchant_name", "SafraPay Merchant"),
                "cnpj": self.config.get("merchant_cnpj", "00000000000000"),
                "address": self.config.get("merchant_address")
            }
            
            logger.info(f"🔍 SafraPay transaction details fetched: {transaction_id}")
            return transaction_data
            
        except Exception as e:
            logger.error(f"❌ Error fetching SafraPay transaction {transaction_id}: {e}")
            raise
