...
const { equipmentStatus, isLoading: equipmentStatusLoading, forceCleanup, isCleaningUp, forceOnline, isForcingOnline } = useEquipmentStatus();
...
<div className="flex justify-between items-center mb-4">
  <h2 className="text-xl font-semibold">Selecione um Equipamento</h2>
                  ...
                  {/* ✅ NOVO: Botão de emergência - Forçar todos offline para online */}
                  <button
                    onClick={() => {
                      console.log('🚨 EMERGÊNCIA - Forçando todos os equipamentos offline para online...');
                      if (confirm('🚨 ATENÇÃO: Esta ação irá forçar TODOS os equipamentos offline para online, mesmo que estejam em uso. Continuar?')) {
                        forceOnline();
                      }
                    }}
                    disabled={isForcingOnline}
                    className={`px-4 py-2 rounded-lg font-medium transition-all duration-200 flex items-center gap-2 ${
                      isForcingOnline 
                        ? 'bg-gray-300 text-gray-500 cursor-not-allowed' 
                        : 'bg-red-500 hover:bg-red-600 text-white shadow-md hover:shadow-lg'
                    }`}
                    title="🚨 EMERGÊNCIA: Forçar todos os equipamentos offline para online"
                  >
                    {isForcingOnline ? (
                      <svg className="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                    ...
                    {isForcingOnline ? 'Forçando...' : '🚨 Forçar Online'}
                  </button>
                  
                  {/* ✅ NOVO: Botão de emergência - Limpar estados inconsistentes */}
                  <button
                    onClick={async () => {
                      console.log('🚨 EMERGÊNCIA - Limpando estados inconsistentes...');
                      if (confirm('🚨 ATENÇÃO: Esta ação irá limpar todos os estados inconsistentes (tickets concluídos mas marcados como em andamento). Continuar?')) {
                        try {
                          await ticketService.emergencyCleanupInconsistentStates();
                          alert('✅ Limpeza de emergência concluída com sucesso!');
                          // Recarregar dados
                          ...
                          alert('❌ Erro na limpeza de emergência. Verifique o console.');
                        }
                      }
                    }}
                    className="px-4 py-2 rounded-lg font-medium transition-all duration-200 flex items-center gap-2 bg-purple-500 hover:bg-purple-600 text-white shadow-md hover:shadow-lg"
                    title="🚨 EMERGÊNCIA: Limpar estados inconsistentes"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                    </svg>
